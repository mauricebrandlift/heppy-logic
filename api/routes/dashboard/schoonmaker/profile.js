// api/routes/dashboard/schoonmaker/profile.js
/**
 * Get schoonmaker profiel data
 * 
 * GET /api/dashboard/schoonmaker/profile
 * 
 * Returns:
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('✅ [Schoonmaker Profile API] Profiel data opgehaald voor user:', authData.id);
    ...
    return res.status(200).json({
      ...
      straatnaam: adresData?.straat || null,
      plaats: adresData?.plaats || null
    });

  } catch (error) {
    console.error('❌ [Schoonmaker Profile API] Error:', error);
