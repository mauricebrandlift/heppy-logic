/**
 * Email Template: Verhuis-/Opleverschoonmaak Bevestiging voor Klant
 * 
 * Wordt verzonden naar klant na succesvolle betaling van verhuis-/opleverschoonmaak.
...
/**
 * Genereer HTML voor verhuis-/opleverschoonmaak bevestiging (Klant)
 * 
 ...
 */
export function verhuisBevestigingKlant(data) {
  const {
    klantNaam,
    plaats,
    uren,
  ...
  const schoonmakerInfo = schoonmakerNaam 
    ? `
      ...
      <p style="font-size: 14px; color: #6b7280; margin-top: 12px; padding: 12px; background: #f0fdf4; border-left: 3px solid #22c55e; border-radius: 4px;">
        <strong>✓ Geen zorgen:</strong> Mocht deze schoonmaker de opdracht niet kunnen accepteren, dan zorgen wij direct voor een geschikte vervanger. Zo garanderen we dat uw verhuis-/opleverschoonmaak op tijd wordt uitgevoerd.
      </p>
    `
    : `
      <div class="warning-badge">⏳ Schoonmaker wordt gezocht</div>
      <p>We zijn momenteel op zoek naar een geschikte schoonmaker voor uw verhuis-/opleverschoonmaak. U ontvangt bericht zodra we iemand hebben gevonden.</p>
    `;
  ...
  const content = `
    ...
    <p>Bedankt voor uw betaling! Uw opdracht voor een verhuis-/opleverschoonmaak is succesvol ontvangen en in behandeling genomen.</p>
    ...
    <ul>
      ...
      <li>🧹 De verhuis-/opleverschoonmaak wordt uitgevoerd op de afgesproken datum en tijd</li>
    ...
    <div style="background: #eff6ff; border-left: 4px solid #3b82f6; padding: 15px; margin: 30px 0; border-radius: 4px;">
      <p style="margin: 0; color: #1e40af;">
        <strong>💡 Tip:</strong> Zorg dat de woning leeg en toegankelijk is. Voor een opleverschoonmaak adviseren we dat alle spullen verwijderd zijn voor het beste resultaat. De schoonmaker neemt eigen schoonmaakmiddelen mee, tenzij u specifieke producten wilt dat hij/zij gebruikt.
      </p>
    </div>
  ...
  return baseLayout(content, 'Verhuis-/Opleverschoonmaak Bevestiging - Heppy Schoonmaak');
}
