// public/forms/bankreiniging/rbsPersoonsgegevensForm.js
// Persoonsgegevens stap voor bank & stoelen reiniging offerte aanvraag
...
function goToFormStep(nextFormName) {
  console.log('[RbsPersoonsgegevens] goToFormStep →', nextFormName);
  if (window.navigateToFormStep) {
    const navigated = window.navigateToFormStep(FORM_NAME, nextFormName);
    if (navigated) {
      console.log('[RbsPersoonsgegevens] navigateToFormStep succesvol', nextFormName);
      return true;
    }
    console.warn('[RbsPersoonsgegevens] navigateToFormStep kon niet navigeren, probeer fallback.');
  }

  if (window.jumpToSlideByFormName) {
    console.log('[RbsPersoonsgegevens] Fallback jumpToSlideByFormName', nextFormName);
    window.jumpToSlideByFormName(nextFormName);
    return true;
  }

  if (window.moveToNextSlide) {
    console.log('[RbsPersoonsgegevens] Fallback moveToNextSlide (geen target match)');
    window.moveToNextSlide();
    return true;
  }

  console.error('[RbsPersoonsgegevens] Geen slider navigatie functie gevonden.');
  return false;
}

export async function initRbsPersoonsgegevensForm() {
  console.log('👤 [RbsPersoonsgegevens] Initialiseren…');
  
  const schema = getFormSchema(FORM_NAME);
  if (!schema) {
    console.error('[RbsPersoonsgegevens] Schema niet gevonden');
    return;
  }
  ...
  console.log('🔍 [RbsPersoonsgegevens] Checking auth state...');
  const authState = authClient.getAuthState();
  const role = authState?.role || 'guest';
  console.log(`👤 [RbsPersoonsgegevens] Auth state detected: ${role}`);
  
  if (authState?.user) {
    console.log('👤 [RbsPersoonsgegevens] User info:', {
      id: authState.user.id,
      email: authState.user.email || authState.user.emailadres,
      role: authState.user.role
    });
  } else {
    console.log('👤 [RbsPersoonsgegevens] Geen authenticated user gevonden');
  }
  ...
  if (role === 'klant' && authState?.user) {
    console.log('🔄 [RbsPersoonsgegevens] Klant detected, starten prefill...');
    await prefillAuthenticatedUser(authState.user);
  } else {
    console.log('ℹ️ [RbsPersoonsgegevens] Guest mode, geen prefill nodig');
  }
  ...
  console.log('👂 [RbsPersoonsgegevens] Luistert naar auth:success events');
  
  // Luister naar auth:state-changed event (na logout)
  document.addEventListener('auth:state-changed', handleAuthStateChanged);
  console.log('👂 [RbsPersoonsgegevens] Luistert naar auth:state-changed events');
  ...
  schema.submit = {
    action: async (formData) => {
      ...
      if (isGuest && formData.emailadres) {
        console.log('🔍 [RbsPersoonsgegevens] Guest aanvraag, checking email beschikbaarheid...');
        console.log('📧 [RbsPersoonsgegevens] Email to check:', formData.emailadres);
        
        try {
          const checkUrl = `${API_CONFIG.BASE_URL}/routes/auth/check-email?email=${encodeURIComponent(formData.emailadres)}`;
          console.log('🌐 [RbsPersoonsgegevens] Fetching:', checkUrl);
          ...
          console.log('📥 [RbsPersoonsgegevens] Response status:', checkResponse.status, checkResponse.ok);
          
          const checkData = await checkResponse.json();
          console.log('📦 [RbsPersoonsgegevens] Response data:', checkData);
          
          if (!checkResponse.ok) {
            console.error('❌ [RbsPersoonsgegevens] API error:', checkResponse.status, checkData);
            // Bij API error: laat door (fail open voor betere UX)
            console.warn('⚠️ [RbsPersoonsgegevens] Continuing despite API error');
            return; // Exit early, laat submit doorgaan
          }
          
          if (checkData.exists === true) {
            console.warn('⚠️ [RbsPersoonsgegevens] Email bestaat al:', formData.emailadres);
          ...
          console.log('✅ [RbsPersoonsgegevens] Email is beschikbaar, continuing...');
        } catch (error) {
          console.error('🔥 [RbsPersoonsgegevens] Catch block:', error.message);
          ...
          if (error.message.includes('e-mailadres is al in gebruik')) {
            console.log('🛑 [RbsPersoonsgegevens] Blocking submit - email exists');
            throw error; // formHandler zal deze message tonen in [data-error-for="global"]
          }
          
          // Netwerk error: log maar block niet
          console.error('❌ [RbsPersoonsgegevens] Email check failed (network):', error);
          console.warn('⚠️ [RbsPersoonsgegevens] Continuing despite email check failure (network issue)');
        }
      } else {
        console.log('ℹ️ [RbsPersoonsgegevens] Skipping email check (not guest or no email):', { isGuest, email: formData.emailadres });
      }
      ...
      console.log('✅ [RbsPersoonsgegevens] Persoonsgegevens opgeslagen:', {
  ...
  schema.shouldValidateField = (fieldName, fieldElement) => {
    ...
    if (!isVisible) {
      console.log(`⏭️ [RbsPersoonsgegevens] Skip validatie voor ${fieldName} (wrapper hidden)`);
    }
    return isVisible;
  };
...
function setupPrevButtonHandler() {
  const prevButton = document.querySelector('[data-form-button-prev="rbs_persoonsgegevens-form"]');
  
  if (!prevButton) {
    console.log('[RbsPersoonsgegevens] Geen prev button gevonden met [data-form-button-prev="rbs_persoonsgegevens-form"]');
    return;
  }
  
  console.log('[RbsPersoonsgegevens] Prev button gevonden, event handler toevoegen...');
  
  // Verwijder oude handler indien aanwezig
  if (prevButtonHandler) {
    prevButton.removeEventListener('click', prevButtonHandler);
    console.log('[RbsPersoonsgegevens] ♻️ Oude prev button handler verwijderd');
  }
  ...
  prevButtonHandler = (e) => {
    ...
    console.log('[RbsPersoonsgegevens] 🔙 Prev button clicked - navigeer naar stap 4 (overzicht)');
    
    // Re-initialiseer de VORIGE stap (stap 4 = bankReinigingOverzichtForm) VOOR navigatie
    import('./bankReinigingOverzichtForm.js').then(module => {
      console.log('[RbsPersoonsgegevens] ♻️ Re-init bankReinigingOverzichtForm voor terug navigatie...');
      module.initBankReinigingOverzichtForm();
      
      // NA re-init, ga naar vorige slide
      if (typeof window.moveToPrevSlide === 'function') {
        console.log('[RbsPersoonsgegevens] Roep window.moveToPrevSlide() aan');
        window.moveToPrevSlide();
      } else {
        console.warn('[RbsPersoonsgegevens] window.moveToPrevSlide() niet beschikbaar');
      }
    }).catch(err => {
      console.error('[RbsPersoonsgegevens] ❌ Fout bij re-init bankReinigingOverzicht:', err);
      if (typeof window.moveToPrevSlide === 'function') {
        window.moveToPrevSlide();
      }
    });
  ...
  console.log('[RbsPersoonsgegevens] ✅ Prev button handler toegevoegd');
}
...
function toggleAuthWrappers(role) {
  console.log(`🔄 [RbsPersoonsgegevens] Toggling wrappers voor role: ${role}`);
  
  const wrappers = document.querySelectorAll('[data-auth-state]');
  console.log(`📦 [RbsPersoonsgegevens] Gevonden ${wrappers.length} wrapper(s) met [data-auth-state]`);
  ...
  wrappers.forEach(wrapper => {
    ...
    if (wrapperRole === role) {
      ...
      visibleCount++;
      console.log(`👁️ [RbsPersoonsgegevens] ✅ Wrapper "${wrapperRole}" GETOOND`);
    } else {
      wrapper.style.display = 'none'; // Verberg
      hiddenCount++;
      console.log(`👁️ [RbsPersoonsgegevens] ❌ Wrapper "${wrapperRole}" verborgen`);
    }
  });
  
  console.log(`📊 [RbsPersoonsgegevens] Wrapper toggle compleet: ${visibleCount} zichtbaar, ${hiddenCount} verborgen`);
}
...
function applyReadonlyFields() {
  console.log('🔒 [RbsPersoonsgegevens] Applying readonly fields...');
  
  const readonlyFields = document.querySelectorAll('[data-readonly="true"]');
  console.log(`📋 [RbsPersoonsgegevens] Gevonden ${readonlyFields.length} readonly veld(en)`);
  ...
  readonlyFields.forEach(field => {
    ...
    console.log(`🔒 [RbsPersoonsgegevens] ✅ Veld "${fieldName}" set to readonly`);
  });
  
  if (readonlyFields.length > 0) {
    console.log('✅ [RbsPersoonsgegevens] Readonly fields toegepast');
  }
}
...
async function prefillAuthenticatedUser(user) {
  console.log('👤 [RbsPersoonsgegevens] === START PREFILL ===');
  console.log('👤 [RbsPersoonsgegevens] User object ontvangen:', {
    id: user?.id,
  ...
  try {
    if (!user) {
      console.warn('⚠️ [RbsPersoonsgegevens] Geen user data beschikbaar');
      return;
    }

    const formEl = document.querySelector(`[data-form-name="${FORM_NAME}"]`);
    if (!formEl) {
      console.warn('⚠️ [RbsPersoonsgegevens] Formulier element niet gevonden voor:', FORM_NAME);
      return;
    }

    console.log('✅ [RbsPersoonsgegevens] Formulier element gevonden');
    
    // Zoek de zichtbare klant wrapper
    const klantWrapper = formEl.querySelector('[data-auth-state="klant"]');
    if (!klantWrapper) {
      console.warn('⚠️ [RbsPersoonsgegevens] Klant wrapper niet gevonden');
      return;
    }
    
    console.log('✅ [RbsPersoonsgegevens] Klant wrapper gevonden');
    ...
    console.log('📋 [RbsPersoonsgegevens] Field map voor prefill:', fieldMap);
    ...
    Object.entries(fieldMap).forEach(([fieldName, value]) => {
      if (value != null && value !== '') {
        ...
        if (field) {
          console.log(`🔍 [RbsPersoonsgegevens] Veld "${fieldName}" gevonden. Current value: "${field.value}", Setting to: "${value}"`);
          field.value = value;
          formHandler.formData[fieldName] = String(value);
          console.log(`🔍 [RbsPersoonsgegevens] Na update - DOM value: "${field.value}", formData: "${formHandler.formData[fieldName]}"`);
          ...
          if (wrapper) {
            ...
            console.log(`🔍 [RbsPersoonsgegevens] Veld "${fieldName}" zit in wrapper "${wrapperState}", display: ${wrapperDisplay}`);
          }
          
          prefilledCount++;
          console.log(`✅ [RbsPersoonsgegevens] Prefilled ${fieldName}: "${value}"`);
        } else {
          console.warn(`⚠️ [RbsPersoonsgegevens] Field niet gevonden: ${fieldName}`);
        }
      } else {
        skippedCount++;
        console.log(`ℹ️ [RbsPersoonsgegevens] Skipped ${fieldName} (geen waarde)`);
      }
    });

    console.log(`📊 [RbsPersoonsgegevens] Prefill stats: ${prefilledCount} gevuld, ${skippedCount} overgeslagen`);
    ...
    console.log('✅ [RbsPersoonsgegevens] Flow data bijgewerkt met authenticated user info');
    
    // Update submit button state na prefill
    if (typeof formHandler.updateSubmitState === 'function') {
      formHandler.updateSubmitState(FORM_NAME);
      console.log('✅ [RbsPersoonsgegevens] Submit state bijgewerkt na prefill');
    }

  } catch (error) {
    console.error('❌ [RbsPersoonsgegevens] Fout bij prefill:', error);
  }
  
  console.log('👤 [RbsPersoonsgegevens] === EINDE PREFILL ===');
}
...
async function handleAuthSuccess(event) {
  console.log('🎉 [RbsPersoonsgegevens] auth:success event ontvangen');
  
  const user = event.detail?.user;
  if (!user) {
    console.warn('⚠️ [RbsPersoonsgegevens] Geen user data in auth:success event');
    return;
  }

  console.log('👤 [RbsPersoonsgegevens] User logged in:', {
    id: user.id,
    email: user.email || user.emailadres,
    role: user.role
  });
  ...
  console.log('✅ [RbsPersoonsgegevens] Auth success afgehandeld');
}
...
function handleAuthStateChanged(event) {
  console.log('🔄 [RbsPersoonsgegevens] auth:state-changed event ontvangen');
  
  const newState = event.detail;
  const role = newState?.role || 'guest';
  
  console.log('👤 [RbsPersoonsgegevens] Nieuwe auth state:', role);
  ...
  if (role === 'guest') {
    ...
    console.log('✅ [RbsPersoonsgegevens] Guest mode: fields gecleared');
  }
}
