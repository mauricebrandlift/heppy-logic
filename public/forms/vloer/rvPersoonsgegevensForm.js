// public/forms/vloer/rvPersoonsgegevensForm.js
// Persoonsgegevens stap voor vloer reiniging offerte aanvraag
...
const FORM_NAME = 'rv_persoonsgegevens-form';
const FLOW_KEY = 'vloer-aanvraag';
...
export async function initrvPersoonsgegevensForm() {
  ...
  schema.submit = {
    ...
    onSuccess: async () => {
      ...
      await logStepCompleted('vloerreinigen', 'persoonsgegevens', 5, {
        flow: FLOW_KEY,
      ...
      try {
        ...
        window.location.href = '/aanvragen/succes/reinigen-vloerreinigen';
...
function setupPrevButtonHandler() {
  const prevButton = document.querySelector('[data-form-button-prev="rv_persoonsgegevens-form"]');
  
  if (!prevButton) {
    console.log('[RtPersoonsgegevens] Geen prev button gevonden met [data-form-button-prev="rv_persoonsgegevens-form"]');
    return;
  }
  ...
  prevButtonHandler = (e) => {
    ...
    console.log('[rvPersoonsgegevens] 🔙 Prev button clicked - navigeer naar stap 4 (overzicht)');
    
    // Re-initialiseer de VORIGE stap (stap 4 = rvOverzichtForm) VOOR navigatie
    import('./rvOverzichtForm.js').then(module => {
      console.log('[rvPersoonsgegevens] ♻️ Re-init rvOverzichtForm voor terug navigatie...');
      module.initRvOverzichtForm();
      
      // NA re-init, ga naar vorige slide
      if (typeof window.moveToPrevSlide === 'function') {
        console.log('[rvPersoonsgegevens] Roep window.moveToPrevSlide() aan');
        window.moveToPrevSlide();
      } else {
        console.warn('[rvPersoonsgegevens] window.moveToPrevSlide() niet beschikbaar');
      }
    }).catch(err => {
      console.error('[rvPersoonsgegevens] ❌ Fout bij re-init rvOverzicht:', err);
      if (typeof window.moveToPrevSlide === 'function') {
        window.moveToPrevSlide();
      }
    });
