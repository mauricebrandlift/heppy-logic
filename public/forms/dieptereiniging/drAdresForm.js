// public/forms/dieptereiniging/drAdresForm.js
// Formulier handling voor stap 1 van de dieptereiniging aanvraag: adresgegevens
...
const FORM_NAME = 'dr_adres-form';
const NEXT_FORM_NAME = 'dr_opdracht-form';

function goToFormStep(nextFormName) {
  console.log('[drAdresForm] goToFormStep →', nextFormName);
  if (window.navigateToFormStep) {
    const navigated = window.navigateToFormStep(FORM_NAME, nextFormName);
    if (navigated) {
      console.log('[drAdresForm] navigateToFormStep succesvol', nextFormName);
      return true;
    }
    console.warn('[drAdresForm] navigateToFormStep kon niet navigeren, probeer fallback.');
  }

  if (window.jumpToSlideByFormName) {
    console.log('[drAdresForm] Fallback jumpToSlideByFormName', nextFormName);
    window.jumpToSlideByFormName(nextFormName);
    return true;
  }

  if (window.moveToNextSlide) {
    console.log('[drAdresForm] Fallback moveToNextSlide (geen target match)');
    window.moveToNextSlide();
    return true;
  }

  console.error('[drAdresForm] Geen slider navigatie functie gevonden.');
  return false;
}

/**
 * Initialiseert het adres formulier voor de dieptereiniging aanvraag.
 * Dit formulier haalt adresgegevens op en controleert de dekkingsstatus.
 */
export function initDrAdresForm() {
  console.log('[drAdresForm] Initialiseren van formulier:', FORM_NAME);
  
  // ✨ Clear andere flow data om vervuiling te voorkomen
  clearOtherFlows('dieptereiniging-aanvraag');
  ...
  if (!schema) {
    console.error(`[drAdresForm] Schema '${FORM_NAME}' niet gevonden in formSchemas.js!`);
    return;
  }
  
  // Laad eventueel bestaande flow data
  const flowData = loadFlowData('dieptereiniging-aanvraag');
  console.log('[drAdresForm] Bestaande flow data:', flowData);
  
  // Definieer de submit actie
  schema.submit = {
    action: async (formData) => {
      // formData wordt door formHandler meegegeven
      console.log('[drAdresForm] Submit action gestart met formData:', formData);
      ...
      try {
        ...
        console.log('[drAdresForm] Adresdetails opgehaald:', addressDetails);
        ...
        console.log('[drAdresForm] Straat en plaats ingevuld. Straat:', addressDetails.straat, 'Plaats:', addressDetails.plaats);

        // Stap 3: Dekking controleren
        const coverageStatus = await fetchCoverageStatus(addressDetails.plaats);
        console.log('[drAdresForm] Dekkingsstatus:', coverageStatus);

        if (coverageStatus && typeof coverageStatus.gedekt === 'boolean') {
          if (coverageStatus.gedekt) {
            console.log(`[drAdresForm] Plaats '${addressDetails.plaats}' is gedekt. Kan doorgaan naar volgende stap.`);
            ...
            logStepCompleted('dieptereiniging', 'adres', 1, {
              postcode,
              huisnummer,
              straat: addressDetails.straat,
              plaats: addressDetails.plaats,
              gedekt: true
            }).catch(err => console.warn('[drAdresForm] Tracking failed:', err));
          } else {
            console.log(`[drAdresForm] Plaats '${addressDetails.plaats}' is NIET gedekt. Zal navigeren naar geen-dekking pagina.`);
        ...
        console.error('[drAdresForm] Fout tijdens submit action:', error);
    ...
    onSuccess: () => {
      console.log('[drAdresForm] Submit action succesvol afgehandeld, nu navigatie uitvoeren');
      
      // Bepaal waar we heen navigeren op basis van de dekkingsstatus
      if (formHandler.formData.heeftDekking) {
        console.log('[drAdresForm] Adres heeft dekking, naar volgende slide...');
        
        // Initialiseer het opdracht formulier vóór de navigatie
        import('./drOpdrachtForm.js').then(module => {
          console.log('[drAdresForm] Stap 2 (drOpdrachtForm) wordt geïnitialiseerd...');
          module.initDrOpdrachtForm();
          
          // Na initialisatie, navigeer naar de volgende slide
          goToFormStep(NEXT_FORM_NAME);
        }).catch(err => {
          console.error('[drAdresForm] Kon stap 2 (drOpdrachtForm) niet laden:', err);
          // Alsnog doorgaan naar volgende slide, ook al is er een probleem met het laden
          goToFormStep(NEXT_FORM_NAME);
        });
      } else {
        console.log('[drAdresForm] Adres heeft geen dekking, navigeren naar geen-dekking pagina...');
  ...
  console.log(`[drAdresForm] Formulier '${FORM_NAME}' is succesvol geïnitialiseerd.`);
}
