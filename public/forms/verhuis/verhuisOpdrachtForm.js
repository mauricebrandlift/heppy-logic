// public/forms/verhuis/verhuisOpdrachtForm.js
// Formulier handling voor stap 2 van de verhuis/opleverschoonmaak aanvraag: opdracht details
...
const FORM_NAME = 'vh_opdracht-form';
const NEXT_FORM_NAME = 'vh_schoonmaker-form';
...
function goToFormStep(nextFormName) {
  console.log('[verhuisOpdrachtForm] goToFormStep →', nextFormName);
  if (window.navigateToFormStep) {
    const navigated = window.navigateToFormStep(FORM_NAME, nextFormName);
    if (navigated) {
      console.log('[verhuisOpdrachtForm] navigateToFormStep succesvol', nextFormName);
      return true;
    }
    console.warn('[verhuisOpdrachtForm] navigateToFormStep kon niet navigeren, probeer fallback.');
  }

  if (window.jumpToSlideByFormName) {
    console.log('[verhuisOpdrachtForm] Fallback jumpToSlideByFormName', nextFormName);
    window.jumpToSlideByFormName(nextFormName);
    return true;
  }

  if (window.moveToNextSlide) {
    console.log('[verhuisOpdrachtForm] Fallback moveToNextSlide (geen target match)');
    window.moveToNextSlide();
    return true;
  }

  console.error('[verhuisOpdrachtForm] Geen slider navigatie functie gevonden.');
  return false;
}
...
async function getPricingConfiguration() {
  ...
  try {
    console.log('🔄 [verhuisOpdrachtForm] Ophalen prijsconfiguratie via API client...');
    const result = await fetchPricingConfiguration('verhuis_opleverschoonmaak');
    const data = result;
    
    console.log('✅ [verhuisOpdrachtForm] Prijsconfiguratie opgehaald:', data);
    ...
    if (Array.isArray(data.pricing) && data.pricing.length > 0) {
      ...
      console.log('✅ [verhuisOpdrachtForm] Prijsconfiguratie verwerkt:', {
      ...
      console.error('❌ [verhuisOpdrachtForm] Geen geldige prijsconfiguratie gevonden in de API respons');
      return false;
    }
  } catch (error) {
    console.error('❌ [verhuisOpdrachtForm] Fout bij ophalen prijsconfiguratie:', error);
    return false;
  } finally {
...
function berekenDagenVooruit(datumString) {
  ...
  console.log(`[verhuisOpdrachtForm] Datum check: ${datumString} = ${dagen} dagen vooruit`);
...
function valideerDatum(datumString, formElement = null, showUI = true) {
  ...
  console.log(`[verhuisOpdrachtForm] Valideer datum: ${datumString}, dagen: ${dagen}, min: ${MIN_DAGEN_VOORUIT}, max: ${MAX_DAGEN_VOORUIT}`);
  
  const datumField = formElement?.querySelector('[data-field-name="vh_datum"]');
  const errorContainer = formElement?.querySelector('[data-error-for="vh_datum"]');
  ...
  if (dagen < MIN_DAGEN_VOORUIT) {
    ...
    console.log('❌ [verhuisOpdrachtForm] Datum te vroeg:', error);
    
    if (showUI && errorContainer) {
      showFieldErrors(formElement, 'vh_datum', error);
  ...
  if (dagen > MAX_DAGEN_VOORUIT) {
    ...
    console.log('❌ [verhuisOpdrachtForm] Datum te ver:', error);
  ...
  console.log('✅ [verhuisOpdrachtForm] Datum geldig:', dagen, 'dagen');
...
function calculateHours(m2, toilets, bathrooms) {
  ...
  console.log(`🧮 [verhuisOpdrachtForm] Berekening: ${m2Num}m² (${m2Minutes}min) + ${toiletsNum} toiletten (${toiletMinutes}min) + ${bathroomsNum} badkamers (${bathroomMinutes}min) = ${totalMinutes}min = ${hours}u`);
...
function updateCalculationUI(formElement) {
  ...
  const urenField = formElement.querySelector('[data-field-total="calculate_form_vh_uren"]');
  if (urenField) {
    urenField.textContent = formatHours(calculation.adjustedHours);
  }
  
  // Update minimum uren weergave
  const minUrenField = formElement.querySelector('[data-field-total="calculate_form__min_vh_uren"]');
  if (minUrenField) {
  ...
  const prijsField = formElement.querySelector('[data-field-total="calculate_form_vh_prijs"]');
  if (prijsField) {
    prijsField.textContent = `${calculation.totaalPrijs.toFixed(2).replace('.', ',')}`;
  }
  
  // Update spoed waarschuwing
  const spoedWarning = formElement.querySelector('[data-spoed-warning="vh_opdracht-form"]');
  if (spoedWarning) {
  ...
  flowData.vh_uren = calculation.adjustedHours.toString();
  flowData.vh_prijs = calculation.totaalPrijs.toFixed(2);
  flowData.vh_min_uren = calculation.minAllowedHours.toString();
  flowData.vh_basis_prijs = calculation.basisPrijs.toFixed(2);
  flowData.vh_spoed_toeslag = calculation.spoedToeslag.toFixed(2);
...
async function performCalculations(formElement) {
  ...
  if (!pricingConfig.isLoaded) {
    ...
    if (!success) {
      console.error('❌ [verhuisOpdrachtForm] Kon berekening niet uitvoeren: prijsconfiguratie niet beschikbaar');
      return;
    }
  }
  
  // Haal waarden op
  const m2 = formElement.querySelector('[data-field-name="vh_m2"]')?.value || 0;
  const toiletten = formElement.querySelector('[data-field-name="vh_toiletten"]')?.value || 0;
  const badkamers = formElement.querySelector('[data-field-name="vh_badkamers"]')?.value || 0;
  const datum = formElement.querySelector('[data-field-name="vh_datum"]')?.value;
  
  console.log('[verhuisOpdrachtForm] Perform calculations:', { m2, toiletten, badkamers, datum });
...
/**
 * Initialiseert het opdracht formulier voor de verhuis/opleverschoonmaak aanvraag
 */
export async function initVerhuisOpdrachtForm() {
  console.log('[verhuisOpdrachtForm] Initialiseren van formulier:', FORM_NAME);
  ...
  if (!schema) {
    console.error(`[verhuisOpdrachtForm] Schema '${FORM_NAME}' niet gevonden in formSchemas.js!`);
    return;
  }
  ...
  if (!formElement) {
    console.error('[verhuisOpdrachtForm] Form element niet gevonden!');
    return;
  }
  
  console.log('[verhuisOpdrachtForm] Formulier gevonden, ophalen prijsconfiguratie...');
  
  // Haal de prijsconfiguratie op
  const success = await getPricingConfiguration();
  if (!success) {
    console.error('❌ [verhuisOpdrachtForm] Kan formulier niet initialiseren: prijsconfiguratie ophalen mislukt');
  ...
  console.log('✅ [verhuisOpdrachtForm] Prijsconfiguratie geladen, koppelen event handlers...');
  
  // Laad eventueel bestaande flow data
  const flowData = loadFlowData('verhuis-aanvraag');
  console.log('[verhuisOpdrachtForm] Bestaande flow data:', flowData);
  
  // Definieer de submit actie
  schema.submit = {
    action: async (formData) => {
      console.log('[verhuisOpdrachtForm] Submit action gestart met formData:', formData);
      
      const { vh_datum, vh_m2, vh_toiletten, vh_badkamers } = formData;
      
      // Valideer datum met UI feedback
      const datumValidatie = valideerDatum(vh_datum, formElement, true);
      if (!datumValidatie.valid) {
        const error = new Error(datumValidatie.error);
        error.code = 'INVALID_DATE';
        error.fieldName = 'vh_datum';
        throw error;
      }
      
      // Check of alle velden ingevuld zijn
      if (!vh_m2 || !vh_toiletten || !vh_badkamers) {
      ...
      const updatedFlowData = loadFlowData('verhuis-aanvraag') || {};
      updatedFlowData.vh_datum = vh_datum;
      updatedFlowData.vh_m2 = vh_m2;
      updatedFlowData.vh_toiletten = vh_toiletten;
      updatedFlowData.vh_badkamers = vh_badkamers;
      updatedFlowData.vh_uren = calculation.adjustedHours.toString();
      updatedFlowData.vh_basis_prijs = calculation.basisPrijs.toFixed(2);
      updatedFlowData.vh_spoed_toeslag = calculation.spoedToeslag.toFixed(2);
      updatedFlowData.vh_totaal_prijs = calculation.totaalPrijs.toFixed(2);
      updatedFlowData.is_spoed = calculation.isSpoed;
      updatedFlowData.dagen_vooruit = datumValidatie.dagen;
      
      saveFlowData('verhuis-aanvraag', updatedFlowData);
      
      console.log('[verhuisOpdrachtForm] Flow data opgeslagen:', updatedFlowData);
      
      // 🎯 TRACK STEP COMPLETION
      logStepCompleted('verhuis_opleverschoonmaak', 'opdracht', 2, {
        datum: vh_datum,
        m2: vh_m2,
        toiletten: vh_toiletten,
        badkamers: vh_badkamers,
        uren: calculation.adjustedHours,
        prijs: calculation.totaalPrijs,
        is_spoed: calculation.isSpoed
      }).catch(err => console.warn('[verhuisOpdrachtForm] Tracking failed:', err));
    },
    
    onSuccess: () => {
      console.log('[verhuisOpdrachtForm] Submit succesvol, navigeer naar volgende stap');
      
      // Import en initialiseer de volgende stap: schoonmaker keuze
      import('./verhuisSchoonmakerForm.js').then(module => {
        console.log('[verhuisOpdrachtForm] Stap 3 (verhuisSchoonmakerForm) wordt geïnitialiseerd...');
        module.initVerhuisSchoonmakerForm();
        goToFormStep(NEXT_FORM_NAME);
      }).catch(err => {
        console.error('[verhuisOpdrachtForm] Kon stap 3 niet laden:', err);
        goToFormStep(NEXT_FORM_NAME);
      });
    }
  };
  ...
  const m2Input = formElement.querySelector('[data-field-name="vh_m2"]');
  const toilettenInput = formElement.querySelector('[data-field-name="vh_toiletten"]');
  const badkamersInput = formElement.querySelector('[data-field-name="vh_badkamers"]');
  const datumInput = formElement.querySelector('[data-field-name="vh_datum"]');
  
  console.log('[verhuisOpdrachtForm] Input elements gevonden:', {
    m2Input: !!m2Input,
  ...
  [m2Input, toilettenInput, badkamersInput].forEach(input => {
    if (input) {
      input.addEventListener('input', () => {
        console.log(`[verhuisOpdrachtForm] Input gewijzigd: ${input.dataset.fieldName} = ${input.value}`);
  ...
  console.log('[verhuisOpdrachtForm] 🔍 Zoeken naar datum input met selector: [data-field-name="vh_datum"]');
  console.log('[verhuisOpdrachtForm] 🔍 Alle inputs in form:', Array.from(formElement.querySelectorAll('input')).map(i => ({
    name: i.name,
    type: i.type,
  ...
  if (datumInput) {
    console.log('[verhuisOpdrachtForm] ✅ Datum input gevonden:', {
      element: datumInput,
    ...
    const handleDatumChange = async () => {
      ...
      console.log(`[verhuisOpdrachtForm] 📅 Datum change event: "${datum}"`);
      
      // Update formHandler data DIRECT
      if (formHandler.formData) {
        formHandler.formData.vh_datum = datum;
        console.log('[verhuisOpdrachtForm] formHandler.formData.vh_datum updated:', datum);
      }
      
      if (datum) {
        const validatie = valideerDatum(datum, formElement, true); // showUI = true
        console.log('[verhuisOpdrachtForm] Datum validatie resultaat:', validatie);
        
        if (validatie.valid) {
          console.log('[verhuisOpdrachtForm] Datum is geldig, voer berekeningen uit...');
          // Update berekeningen (voor spoed check)
          await performCalculations(formElement);
        } else {
          console.warn('[verhuisOpdrachtForm] Datum is NIET geldig:', validatie.error);
        }
      } else {
        console.warn('[verhuisOpdrachtForm] Datum is leeg');
      }
    };
    ...
    console.log('[verhuisOpdrachtForm] ✅ Datum event listeners toegevoegd');
  } else {
    console.error('[verhuisOpdrachtForm] ❌ Datum input NIET gevonden met selector [data-field-name="vh_datum"]');
    console.error('[verhuisOpdrachtForm] ❌ Check of de HTML data-field-name attribute correct is (geen komma, spaties, etc)');
  }
  ...
  if (increaseBtn) {
    increaseBtn.addEventListener('click', (e) => {
      e.preventDefault();
      console.log('[verhuisOpdrachtForm] Uren verhogen (+0.5)');
  ...
  if (decreaseBtn) {
    decreaseBtn.addEventListener('click', (e) => {
      e.preventDefault();
      console.log('[verhuisOpdrachtForm] Uren verlagen (-0.5)');
      ...
      console.log(`[verhuisOpdrachtForm] Nieuwe uren: ${calculation.adjustedHours} (min: ${minHours})`);
  ...
  const hasPrefillData = formHandler.formData.vh_m2 || formHandler.formData.vh_toiletten || formHandler.formData.vh_badkamers;
  
  console.log('[verhuisOpdrachtForm] FormHandler formData na init:', {
    vh_m2: formHandler.formData.vh_m2,
    vh_toiletten: formHandler.formData.vh_toiletten,
    vh_badkamers: formHandler.formData.vh_badkamers,
    vh_datum: formHandler.formData.vh_datum,
    hasPrefillData
  });
  
  // Als er prefill data is, voer dan direct berekening uit
  if (hasPrefillData) {
    console.log('[verhuisOpdrachtForm] Prefill data gevonden, voer initiële berekening uit...');
    await performCalculations(formElement);
  } else {
    // Toon standaard minimum uren als er nog geen invoer is
    console.log('[verhuisOpdrachtForm] Geen prefill data, toon standaard minimum uren');
    const minUrenField = formElement.querySelector('[data-field-total="calculate_form__min_vh_uren"]');
    if (minUrenField) {
  ...
  console.log(`✅ [verhuisOpdrachtForm] Formulier '${FORM_NAME}' is succesvol geïnitialiseerd.`);
...
function setupPrevButtonHandler() {
  const prevButton = document.querySelector('[data-form-button-prev="vh_opdracht-form"]');
  
  if (!prevButton) {
    console.log('[verhuisOpdrachtForm] Geen prev button gevonden met [data-form-button-prev="vh_opdracht-form"]');
    return;
  }
  
  console.log('[verhuisOpdrachtForm] Prev button gevonden, event handler toevoegen...');
  
  // Verwijder oude handler indien aanwezig
  if (prevButtonHandler) {
    prevButton.removeEventListener('click', prevButtonHandler);
    console.log('[verhuisOpdrachtForm] ♻️ Oude prev button handler verwijderd');
  }
  ...
  prevButtonHandler = (e) => {
    ...
    console.log('[verhuisOpdrachtForm] 🔙 Prev button clicked - navigeer naar stap 1 (adres)');
    
    // Re-initialiseer de VORIGE stap (stap 1 = verhuisAdresForm) VOOR navigatie
    import('./verhuisAdresForm.js').then(module => {
      console.log('[verhuisOpdrachtForm] ♻️ Re-init verhuisAdresForm voor terug navigatie...');
      module.initVerhuisAdresForm();
      
      // NA re-init, ga naar vorige slide via Webflow functie
      if (typeof window.moveToPrevSlide === 'function') {
        console.log('[verhuisOpdrachtForm] Roep window.moveToPrevSlide() aan');
        window.moveToPrevSlide();
      } else {
        console.warn('[verhuisOpdrachtForm] window.moveToPrevSlide() niet beschikbaar, probeer fallback');
      ...
      console.error('[verhuisOpdrachtForm] ❌ Fout bij re-init verhuisAdresForm:', err);
  ...
  console.log('[verhuisOpdrachtForm] ✅ Prev button handler toegevoegd');
}
